...
export default function CommunityChat({ userId, firstName }) {
  ...
  const connect = useCallback(() => {
    ...
    const ws = new WebSocket(
      `${WS_BASE}/ws/chat?user_id=${userId}&first_name=${encodeURIComponent(firstName || 'User')}`
    );
  ...
  return (
    <div style={{ position: 'fixed', bottom: '24px', left: '24px', zIndex: 9000, fontFamily: '-apple-system, BlinkMacSystemFont, system-ui, sans-serif' }}>
      ...
      {/* Chat panel */}
      {open && (
        <div style={{ position: 'absolute', bottom: '56px', left: 0, width: '320px', height: '440px', background: '#080808', border: '1px solid #1a1a1a', borderRadius: '16px', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 8px 40px rgba(0,0,0,0.6)' }}>

          {/* Header */}
          <div style={{ padding: '12px 16px', borderBottom: '1px solid #111', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
            ...
            <button onClick={() => setOpen(false)} style={{ background: 'none', border: 'none', color: '#444', cursor: 'pointer', fontSize: '1rem', lineHeight: 1, padding: '2px 6px' }}>×</button>
          </div>

          {/* Messages */}
          <div style={{ flex: 1, overflowY: 'auto', padding: '12px', display: 'flex', flexDirection: 'column', gap: '10px' }}
            css={`&::-webkit-scrollbar{width:3px}&::-webkit-scrollbar-thumb{background:#1e1e1e;border-radius:2px}`}>
            {messages.length === 0 && (
              <div style={{ textAlign: 'center', color: '#2a2a2a', fontSize: '0.75rem', marginTop: '40px' }}>No messages yet. Say hi!</div>
            )}
            {messages.map((m, i) => {
              const isMe = m.user_id === userId || String(m.user_id) === String(userId);
              return (
                <div key={m.id || i} style={{ display: 'flex', flexDirection: isMe ? 'row-reverse' : 'row', alignItems: 'flex-end', gap: '7px' }}>
                  ...
                  <div style={{ maxWidth: '72%' }}>
                    ...
                    <div style={{ background: isMe ? '#ff3333' : '#111', color: isMe ? '#fff' : '#ccc', padding: '7px 11px', borderRadius: isMe ? '12px 12px 3px 12px' : '12px 12px 12px 3px', fontSize: '0.82rem', lineHeight: 1.45, wordBreak: 'break-word' }}>
                      {m.message}
                    </div>
          ...
          <div style={{ padding: '10px 12px', borderTop: '1px solid #111', display: 'flex', gap: '8px', flexShrink: 0 }}>
            ...
            <button
              ...
              style={{ background: input.trim() && connected ? '#ff3333' : '#111', border: 'none', borderRadius: '10px', color: input.trim() && connected ? '#fff' : '#333', width: '36px', cursor: input.trim() && connected ? 'pointer' : 'default', fontSize: '1rem', transition: 'all 0.15s', flexShrink: 0 }}>
              ↑
            </button>
          </div>
        </div>
